...
int __cdecl mesDispWindowHook(int a1, int a2, int a3, int a4, int a5, int a6,
  ...
  if (*MouseEnabled || AutoSkipAlphaStep) {
    ...
    if (!LockMouseControls) {
      if ((gameExeScrWork[SW_GAMEMODE] & 1) && gameExeGetFlag(CALENDAR_DISP) &&
          !gameExeGetFlag(Pokecom_Disable) && !gameExeGetFlag(Pokecom_Open)) {
      ...
      if (mouseSelectHitTest(mouseX, mouseY, 34, 1010, 155, 45)) {
        drawSpriteHook(80, 3837.0f, 1180.0f, 155.0f, 45.0f, 34.0f, 966.0f,
                       0xFFFFFF, alpha, 1);
        if (InputObject->mouseButtons & MouseLeftClick) {
          CarryInputToTheNextFrame |= PAD1X;
        }
      } else {
        drawSpriteHook(80, 3637.0f, 1180.0f, 155.0f, 45.0f, 34.0f, 966.0f,
                       0xFFFFFF, alpha, 1);
      }

      if (mouseSelectHitTest(mouseX, mouseY, 65, 1063, 155, 45)) {
        drawSpriteHook(80, 3868.0f, 1234.0f, 155.0f, 45.0f, 65.0f, 1020.0f,
                       0xFFFFFF, alpha, 1);
        if (InputObject->mouseButtons & MouseLeftClick) {
        ...
        drawSpriteHook(80, 3668.0f, 1234.0f, 155.0f, 45.0f, 65.0f, 1020.0f,
                       0xFFFFFF, alpha, 1);
      }
    }
  }
...
int __cdecl instOptionHook(uint8_t* thread) {
  ...
  if (*MouseEnabled && type == 1) {
    ...
    mouseSlider(mouseX, mouseY, 163, 469, 156, 30, 256, 4096,
                *ConfigMessageSpeed, 0);
    *ConfigMessageSpeedCur = *ConfigMessageSpeed;
    mouseSlider(mouseX, mouseY, 163, 525, 156, 30, 2048, 256,
                *ConfigAutoModeDelay, 1);
    *ConfigAutoModeDelayCur = *ConfigAutoModeDelay;
    mouseSlider(mouseX, mouseY, 163, 697, 156, 30, 0, 128, *ConfigVoiceVolume,
                2);
    ...
    mouseSlider(mouseX, mouseY, 163, 865, 156, 30, 0, 128, *ConfigMovieVolume,
                5);

    for (int i = 13; i < 28; i++) {
      int* val = (int*)ConfigUnkArray[9 * i + 6];
      mouseSlider(mouseX, mouseY, 914, 259 + ((i - 13) * 50), 159, 25, 0, 128,
                  *val, i);
    }
    ...
    if (!SliderMoving) {
      ...
      for (int i = 0; i < 3; i++) {
        if (index == 1) index++;
        if (menuButtonHitTest(index++, mouseX, mouseY, 149, 249 + (i * 56), 637,
                              48, ConfigSelectedItem) &&
            InputObject->mouseButtons & MouseLeftClick) {
          gameExePlaySE(18);
      ...
      for (int i = 0; i < 3; i++) {
        if (menuButtonHitTest(index++, mouseX, mouseY, 149, 477 + (i * 56), 637,
                              48, ConfigSelectedItem) &&
            InputObject->mouseButtons & MouseLeftClick) {
          if (index == 7) {
            gameExePlaySE(18);
      ...
      for (int i = 0; i < 6; i++) {
        if (menuButtonHitTest(index++, mouseX, mouseY, 149, 706 + (i * 56), 637,
                              48, ConfigSelectedItem) &&
            InputObject->mouseButtons & MouseLeftClick) {
      ...
      for (int i = 0; i < 15; i++) {
        if (menuButtonHitTest(index++, mouseX, mouseY, 904, 265 + (i * 50), 433,
                              41, ConfigSelectedItem)) {
          if (mouseHitTest(mouseX, mouseY, 1089, 265 + (i * 50), 247, 41) &&
              InputObject->mouseButtons & MouseLeftClick) {
            *InputMask |= PAD1Y;
            gameExePlaySE(18);
          }
...
int __cdecl pokecomARMainHook() {
  if (*MouseEnabled && gameExeGetFlag(Pokecom_ManualMode)) {
    ...
    if (*ARNumDisplayedGeoTags) {
      for (int i = 0; i < *ARNumDisplayedGeoTags; i++) {
        ...
        gameExeGetScreenCoords(1, &x, &y, ARGeoTagsXYZCoords[xyzCnt],
                               1.25f + ARGeoTagsXYZCoords[xyzCnt + 1],
        ...
        if (menuButtonHitTest(id, mouseX, mouseY, screenX, screenY + 62,
                              (textWidth + 80) * pokecomScaleX, 62,
                              ARSelectedGeoTag)) {
          if (InputObject->mouseButtons & MouseLeftClick) {
    ...
    if (gameExeGetFlag(KRep_SearchMode) && *ARPointingAtReport) {
      ...
      if (mouseSelectHitTest(mouseX, mouseY, screenX, screenY,
                             72 * pokecomScaleX, 70 * pokecomScaleY) &&
          (InputObject->mouseButtons & MouseLeftClick))
        *InputMask |= PAD1A;
    }
    ...
    if ((int8_t)gameExeScrWork[SW_MAP_MODE] < 0) {
      for (int i = 0; i < *MapNumPoints; i++) {
        if (MapPointIsDisplayed[i]) {
          ...
          int screenX =
              1920 *
                  (MapPointsData[5 * id + 1] -
                   (gameExeScrWork[SW_MAP_POSX] -
                    (1920 * gameExeScrWork[SW_MAP_SIZE] / 1000) / 2)) /
                  (1920 * gameExeScrWork[SW_MAP_SIZE] / 1000) -
              84;
          int screenY =
              1080 *
                  (MapPointsData[5 * id + 2] -
                   (gameExeScrWork[SW_MAP_POSY] -
                    (1080 * gameExeScrWork[SW_MAP_SIZE] / 1000) / 2)) /
                  (1080 * gameExeScrWork[SW_MAP_SIZE] / 1000) -
              138;

          int height = 93;
          int width = 150;
          width += (2 * MapPointsData[5 * id + 3] + 54) > 96
                       ? 2 * MapPointsData[5 * id + 3] - 42
                       : 0;
          if (!gameExeScrWork[6377]) {
            auto pokecomWidth =
                *(signed __int16*)((char*)PokecomWindowCoords +
                                   (16 * gameExeScrWork[6374]) + 12);
            auto pokecomHeight =
                *(signed __int16*)((char*)PokecomWindowCoords +
                                   (16 * gameExeScrWork[6374]) + 14);
          ...
          if (menuButtonHitTest(i, mouseX, mouseY, screenX, screenY, width,
                                height, MapSelectedPointIndex)) {
            if (InputObject->mouseButtons & MouseLeftClick) {
              *InputMask |= PAD1A;
            }
          }
...
void __cdecl twipoMainHook() {
  if (*MouseEnabled) {
    ...
    if (gameExeScrWork[SW_TWIPOMODE] == 0) {
      // Twipo tabs
      for (int i = 0; i < 4; i++) {
        if (mouseSelectHitTest(mouseX, mouseY, 449, 385 + (i * 157), 59, 157) &&
            (InputObject->mouseButtons & MouseLeftClick))
      ...
      if ((TwipoTweepCountInTabs[gameExeScrWork[SW_TWIPOCURTAB]] -
        ...
        if (mouseSelectHitTest(mouseX, mouseY, 508, 437, 383, 209) &&
            (InputObject->mouseButtons & MouseLeftClick)) {
          *InputMask4 |= PAD1RIGHTSTICKUP;
        }
      }
      if (gameExeScrWork[SW_TWIPOCURTW] > 0) {
        if (mouseSelectHitTest(mouseX, mouseY, 508, 849, 383, 209) &&
            (InputObject->mouseButtons & MouseLeftClick)) {
...
int __cdecl backlogMenuMainHook() {
  if (*MouseEnabled) {
    ...
    int scrollBarY =
        *BacklogDispPosMax <= 600
            ? 138
            : (793 * *BacklogDispPos / (*BacklogDispPosMax - 600)) + 138;
    mouseScrollBar(mouseX, mouseY, 1820, scrollBarY + 48, 11, 48, 0,
                   *BacklogDispPosMax - 600, 138, 931, *BacklogDispPos, 0);

    if (!SliderMoving) {
      int startLineIndex =
          *BacklogFirstFullDispLine > 0 ? *BacklogFirstFullDispLine - 1 : 0;
      for (int i = startLineIndex; i < *BacklogLastFullDispLine + 2; i++) {
        int lineY = (float)(BacklogDispLinePosY[i] - *BacklogDispPos + 54 +
                            BacklogDispLineSize[i] + 16) *
                    1.5f;
        if (menuButtonHitTest(i, mouseX, mouseY, 538, lineY, 1725,
                              (BacklogDispLineSize[i] + 16) * 1.5f,
      ...
      if (ScrollDownToCloseBacklog &&
          (*BacklogDispPos >= *BacklogDispPosMax - 600) &&
      ...
      if (InputObject->mouseButtonsHeld & MouseScrollWheelUp) {
        ...
        *BacklogDispPos = *BacklogDispPos + 48 >= *BacklogDispPosMax - 608
                              ? *BacklogDispPosMax - 600
                              : *BacklogDispPos + 48;
        gameExeBacklogRecalcMovement();
      }
...
int __cdecl musicMenuMainHook() {
  if (*MouseEnabled) {
    ...
    mouseScrollBar(mouseX, mouseY, 1815, scrollBarY + 60, 10, 60, 0, 22, 357,
                   909, *MusicModeStartIndex, 0);

    if (!SliderMoving) {
      for (int i = 0; i < 16; i++) {
        if (menuButtonHitTest(i, mouseX, mouseY, 80, 399 + (i * 38), 1690, 40,
                              MusicModeCurrentIndex) &&
            (InputObject->mouseButtons & MouseLeftClick))
          *InputMask |= PAD1A;
      }

      if (mouseSelectHitTest(mouseX, mouseY, 1600, 198, 222, 22) &&
          (InputObject->mouseButtons & MouseLeftClick))
        *InputMask |= PAD1Y;
      ...
      if (InputObject->mouseButtonsHeld & MouseScrollWheelUp) {
        ...
        *MusicModeStartIndex =
            *MusicModeStartIndex == 22 ? 22 : *MusicModeStartIndex + 1;
      }
...
void __cdecl pokecomMainMenuMainHook() {
  if (*MouseEnabled) {
    ...
    if (menuButtonHitTest(0, mouseX, mouseY, 408, 382, 275, 154,
                          PokecomSelectionIndex) ||
        menuButtonHitTest(1, mouseX, mouseY, 684, 382, 275, 154,
                          PokecomSelectionIndex) ||
        menuButtonHitTest(3, mouseX, mouseY, 1236, 382, 275, 154,
                          PokecomSelectionIndex) ||
        menuButtonHitTest(7, mouseX, mouseY, 1236, 538, 275, 154,
                          PokecomSelectionIndex) ||
        menuButtonHitTest(9, mouseX, mouseY, 684, 693, 275, 154,
                          PokecomSelectionIndex) ||
        menuButtonHitTest(10, mouseX, mouseY, 960, 693, 275, 154,
                          PokecomSelectionIndex) ||
        menuButtonHitTest(12, mouseX, mouseY, 408, 849, 275, 154,
                          PokecomSelectionIndex) ||
        menuButtonHitTest(14, mouseX, mouseY, 960, 849, 275, 154,
                          PokecomSelectionIndex) ||
        menuButtonHitTest(15, mouseX, mouseY, 1236, 849, 275, 154,
                          PokecomSelectionIndex)) {
      if (InputObject->mouseButtons & MouseLeftClick) {
        *InputMask |= PAD1A;
      }
...
int __cdecl albumMenuMainHook() {
  if (*MouseEnabled) {
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 4; j++) {
        if (menuButtonHitTest(id++, mouseX, mouseY, 128 + (j * 426),
                              427 + (i * 257), 386, 218, CGLibraryItemIndex) &&
            (InputObject->mouseButtons & MouseLeftClick))
          *InputMask |= PAD1A;
      }
    }
    ...
    if (InputObject->mouseButtonsHeld & MouseScrollWheelUp) {
      *CGLibraryPageIndex =
          *CGLibraryPageIndex == 0 ? 7 : *CGLibraryPageIndex - 1;
    } else if (InputObject->mouseButtonsHeld & MouseScrollWheelDown) {
      *CGLibraryPageIndex =
          *CGLibraryPageIndex == 7 ? 0 : *CGLibraryPageIndex + 1;
    }
...
int __cdecl instTipsHook(void* thread) {
  if (*MouseEnabled) {
    ...
    if (TipsMenuTipData[6 * *TipsMenuSelectedTip] & 1 &&
      ...
      int scrollBarY =
          530 * *TipsMenuTipTextStartOffset / (*TipMenuTipTextHeight - 370) +
          374;
      mouseScrollBar(mouseX, mouseY, 1822, scrollBarY + 60, 10, 60, 0,
                     *TipMenuTipTextHeight - 370, 376, 906,
                     *TipsMenuTipTextStartOffset, 0);
    }

    if (TipsMenuTabTipCount[*TipsMenuCurrentTab] > 22) {
      int scrollBarY = (671 * *TipsMenuStartIndex /
                        (TipsMenuTabTipCount[*TipsMenuCurrentTab] - 22)) +
                       222;
      mouseScrollBar(mouseX, mouseY, 555, scrollBarY + 60, 10, 60, 0,
                     TipsMenuTabTipCount[*TipsMenuCurrentTab] - 22, 224, 895,
                     *TipsMenuStartIndex, 1);
      *((float*)(TipsMenuStartIndex + 3)) = (float)*TipsMenuStartIndex;
    }

    if (!SliderMoving) {
      int count = TipsMenuTabTipCount[*TipsMenuCurrentTab] > 22
                      ? 22
                      : TipsMenuTabTipCount[*TipsMenuCurrentTab];
      for (int i = 0; i < count; i++) {
        if (menuButtonHitTest(i, mouseX, mouseY, 82, 287 + (i * 32), 467, 32,
      ...
      for (int i = 0; i < 4; i++) {
        if (TipsMenuTabTipCount[i]) {
          if (mouseSelectHitTest(mouseX, mouseY, 80 + (i * 126), 189, 126,
                                 30) &&
              (InputObject->mouseButtons & MouseLeftClick))
...
int __cdecl instSystemMenuHook(void* thread) {
  if (*MouseEnabled) {
    ...
    for (int i = 0; i < 11; i++) {
      if (menuButtonHitTest(i, mouseX, mouseY, 40 + (i * 48), 308 + (i * 77),
                            766, 65, &gameExeScrWork[SW_SYSMENUSELNO]) &&
          (InputObject->mouseButtons & MouseLeftClick))
        *InputMask |= PAD1A;
    }
...
int __cdecl instSaveMenuHook(uint8_t* thread) {
  if (*MouseEnabled) {
    ...
    if (type == 11) {  // Daily Records/Data Collection
      ...
      for (int i = 0; i < 6; i++) {
        for (int j = 0; j < 7; j++) {
          if (menuButtonHitTest(id++, mouseX, mouseY, 378 + (j * 202),
                                349 + (i * 123), 198, 122, selIndexVar) &&
              (InputObject->mouseButtons & MouseLeftClick))
            *InputMask |= PAD1A;
        }
      }
      ...
      for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 4; j++) {
          if (menuButtonHitTest(id++, mouseX, mouseY, 56 + (i * 910),
                                365 + (j * 201), 896, 178,
...
int __cdecl movieModeDispHook(void* thread) {
  if (*MouseEnabled) {
    ...
    if (!*MovieModeIsPlaying) {
      ...
      for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 3; j++) {
          if (menuButtonHitTest(id++, mouseX, mouseY, 143 + (j * 560),
                                560 + (i * 336), 514, 291,
                                MovieModeSelectionIndex) &&
              (InputObject->mouseButtons & MouseLeftClick))
            *InputMask |= PAD1A;
      ...
      if (gameExeScrWork[SW_GAMEMODE] & 1) {
        if (!mouseHitTest(InputObject->scaledMouseX, InputObject->scaledMouseY,
            ...
            !(mouseSelectHitTest(InputObject->scaledMouseX,
              ...
              !gameExeGetFlag(Pokecom_Disable) &&
