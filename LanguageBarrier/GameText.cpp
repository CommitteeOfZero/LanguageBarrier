  ...
  void __cdecl funcName(int fontNumber, int pageNumber, uint32_t opacity,      \
		...
		for (int i = 0; i < page->pageLength; i++) {
			if (fontNumber == page->fontNumber[i]) {
				...
				if (page->charOutlineColor[i] != -1) {
					...
					{
						uint32_t currentChar = page->glyphCol[i] + page->glyphRow[i] * 64;
						const auto& glyphInfo = TextRendering::Instance().getFont(page->glyphDisplayHeight[i] * 1.5f)->glyphData.outlineMap[TextRendering::Instance().charMap[currentChar]];
				...
				{
					uint32_t currentChar = page->glyphCol[i] + page->glyphRow[i] * 4096 / 64;
				...
				default:
					...
					if (!measureOnly) {
						...
						if (!TextRendering::Instance().enabled) {
							...
							result->displayStartX[i] =
								(xOffset * 2.0f + (curLineLength)) * multiplier;
							result->displayStartY[i] =
								(yOffset * 2.0f + (result->lines * lineHeight)) * multiplier;
							result->displayEndX[i] = (xOffset * 2.0f + curLineLength + glyphInfo->width) * multiplier;
							result->displayEndY[i] =
								(yOffset * 2.0f + (result->lines) * lineHeight + baseGlyphSize) * multiplier;
							result->color[i] = currentColor;
						}
					}
	...
	int __cdecl rnDrawTextHook(signed int textureId, int a2, signed int startY, unsigned int a4, uint8_t* sc3, signed int startX, int color, int height, int opacity)
	{
		...
		if (TextRendering::Instance().enabled) {
			...
			for (int i = 0; i < v.size(); i++) {
				...
				int column = currentChar % 64;
				int row = currentChar / 64;
	...
	int setTipContentHook(char* sc3string) {
		...
		processSc3TokenList(0, 0, TIP_REIMPL_LINE_LENGTH, words, 255, 0,
			TIP_REIMPL_GLYPH_SIZE, &str, false, 1, -1, NOT_A_LINK, 0, TIP_REIMPL_GLYPH_SIZE);
	...
	void drawTipContentHook(int textureId, int maskId, int startX, int startY,
		...
		processSc3TokenList(startX, startY, TIP_REIMPL_LINE_LENGTH, words, 255, color,
			...
			NOT_A_LINK, color, TIP_REIMPL_GLYPH_SIZE * 1.5f);
		maskHeight *= 1.5f;
		for (int i = 0; i < str.length; i++) {
